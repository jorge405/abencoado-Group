...
export default{
    ...
    methods:{
        ...
        async envioLibro(){
            ...
            const [anio,mes,dia]= this.fecha_comprobante.split('-')
                const datos={
                    ...
                    glosa:this.glosa,
                    metodo_pago:this.metodo_pago.toLocaleLowerCase(),
                    total_debe:this.total_debe,
                    total_haber:this.total_haber,
                    estado:'activo',
                    dolar:Number(this.dolar).toFixed(2),
                    ufv:Number(this.ufv).toFixed(2),
                    anio:anio,
    ...
    watch:{
        async tipo_comprobante(newval, oldval){
            ...
            if (this.fecha_comprobante==='') {
                ...
                const [ano,mes,dia] = this.fecha_comprobante.split('-');
                const fecha= new Date(ano,mes -1,dia);
                const mesFormato= (fecha.getMonth()+1).toString().padStart(2,'0');
                const nroComprobante= await this.getComprobante(this.fecha_comprobante,newval);
                this.nro_comprobante=`${tipo_comprobante}${mesFormato}${nroComprobante}`
            }
        ...
        async fecha_comprobante(newval){
            ...
            const fecha = new Date(ano, mes - 1, dia);
            const tipo_comprobante= this.tipo_comprobante==='ingreso' ? 1 : this.tipo_comprobante==='egreso' ? 2 : 3;
            const diaFormato = fecha.getDate().toString().padStart(2,'0');
            const mesFormato = (fecha.getMonth() + 1).toString().padStart(2,'0');
            const anoFormato = fecha.getFullYear();
            const nroComprobante= await this.getComprobante(newval,this.tipo_comprobante)
    ...
    <div class="fixed inset-0 flex items-center justify-around ml-40 z-50" v-if="mostrarRegistro">
        ...
        <div class=" grid grid-cols-2 space-x-2 ">
            ...
            <div class=" flex flex-col">
                ...
                <input v-model="nro_comprobante" disabled :class="{'bg-gray-400' :mostrarRegistro, 'bg-gray-400' :!mostrarRegistro} "  type="text" class=" bg-white text-sm p-1.5 rounded-xl border border-gray-200  placeholder:text-sm focus:border-sky-300 focus:outline-hidden focus:ring-3 focus:ring-sky-400/10" placeholder="ingrese nro comprobante">
            </div>
            <div class=" flex flex-col">
        ...
        <div class=" flex flex-row space-x-2 items-center mt-5">
            ...
            <div class=" flex flex-col">
                ...
                <input v-model="dolar" type="text" class=" bg-white w-28 text-sm p-2 rounded-xl border border-gray-200  placeholder:text-sm focus:border-sky-300 focus:outline-hidden focus:ring-3 focus:ring-sky-400/10" placeholder="$us">            
            </div>
            <div class=" flex flex-col">
                <label class=" block font-Nunito text-xs 2xltext-sm text-slate-700">T.C Ufv</label>
                <input v-model="ufv" type="text" class=" bg-white w-28 text-sm p-2 rounded-xl border border-gray-200  placeholder:text-sm focus:border-sky-300 focus:outline-hidden focus:ring-3 focus:ring-sky-400/10" placeholder="UFV">            
            </div>
        </div>
        </div>
        <div class="grid grid-cols-3 space-x-4">
            <div class=" flex flex-col">
                ...
                <input v-model="selectedEmpresa" type="text" class=" bg-white  text-sm p-2 rounded-xl border border-gray-200  placeholder:text-sm focus:border-sky-300 focus:outline-hidden focus:ring-3 focus:ring-sky-400/10" placeholder="razon social">            
            </div>
            <div class=" flex flex-col">
        ...
        <div class=" flex flex-row items-center space-x-4 mr-8 justify-end">
            ...
            <input  type="text" disabled :value="total_debe.toFixed(2)" class=" bg-white text-sm  rounded-xl border border-gray-300 p-1.5 placeholder:text-sm placeholder:text-slate-900 focus:border-sky-300 focus:outline-hidden focus:ring-3 focus:ring-sky-400/10 text-slate-900  " placeholder="">
            <input   type="text" disabled :value="total_haber.toFixed(2)" class=" bg-white text-sm rounded-xl border border-gray-300 p-1.5 placeholder:text-sm placeholder:text-slate-900 focus:border-sky-300 focus:outline-hidden focus:ring-3 focus:ring-sky-400/10 text-slate-900  " placeholder="">
