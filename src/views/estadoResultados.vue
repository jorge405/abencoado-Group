...
export default{
    data(){
        return{
            ...
            listaEstadoResultados:[],
            totales:{ingresos:0,gastos:0,utilidad:0},
            key:'abencoadoGroup',
    ...
    methods:{
        async estadoResultado(){
            ...
            try {
                ...
                if (responseEstadoResultado.data.status==='vacio') {
                    ...
                    this.listaEstadoResultados = responseEstadoResultado.data.detalles;
                    this.totales = responseEstadoResultado.data.totales;
        ...
        mostrarSumaYsaldo(){
            ...
            this.MostrarEstadoResultado=false;
        },
...
<template>
  <sidebar>
    <template v-if="MostrarEstadoResultado" #title>Estado de Resultados</template>
    <div class="flex flex-col w-6xl bg-transparent rounded-xl p-2 ml-45" v-if="MostrarEstadoResultado">
      <div class="flex flex-col bg-white border border-gray-300 mx-auto w-5xl min-h-11/12 mt-5 rounded-lg shadow-lg">
        ...
        <div class="flex flex-row px-4 mt-4">
                ...
                <button  class=" p-1.5 text-sm text-white bg-blue-950 rounded-lg font-Nunito cursor-pointer">Estado de Resultados</button>
        ...
        <div class="flex-1 mt-7 overflow-y-auto px-4 max-h-120" id="tablaEstadoResultado">
          <table class="w-full text-sm text-left border-collapse">
            ...
            <tbody>
              <tr v-if="!listaEstadoResultados.length">
                <td colspan="3" class="text-center py-10 text-gray-400">No se encontraron datos</td>
              </tr>

              <tr v-for="(item, index) in listaEstadoResultados" 
                  :key="index" 
        ...
        <div class=" grid grid-cols-2 mx-10">
            <div class="flex flex-row my-10 space-x-4 border border-gray-300 p-4 ">
                <div class="flex flex-col justify-center items-center space-y-2">
                    <label class="text-xs font-nunito text-slate-900 font-semibold">INGRESO</label>
                    <input :value="totales.ingresos" type="text" disabled class=" w-20 bg-white text-sm p-2 rounded-xl border border-gray-200  placeholder:text-xs focus:border-sky-300 focus:outline-hidden focus:ring-3 focus:ring-sky-400/10" placeholder="">
                </div>
                <div class="flex flex-col justify-center items-center space-y-2">
                    <label class="text-xs font-nunito text-slate-900 font-semibold">GASTO</label>
                    <input :value="totales.gastos" type="text" disabled class=" w-20 bg-white text-sm p-2 rounded-xl border border-gray-200  placeholder:text-xs focus:border-sky-300 focus:outline-hidden focus:ring-3 focus:ring-sky-400/10" placeholder="">
                </div>
            </div>
            <div class="flex flex-row ml-10 my-10 space-x-4 border border-gray-300 p-4 ">
                
                <div class="flex flex-row justify-center items-center space-x-2">
                    <label class="text-xs font-nunito text-slate-900 font-semibold">UTILIDAD =INGRESO -GASTO</label>
                    <input :value="totales.utilidad"  type="text" disabled class=" w-20 bg-white text-sm p-2 rounded-xl border border-gray-200  placeholder:text-xs focus:border-sky-300 focus:outline-hidden focus:ring-3 focus:ring-sky-400/10" placeholder="">
    ...
    <transition enter-active-class="transition duration-300 ease-out"
        ...
        <div class="bg-gray-50 bg-opacity-80 text-white  ml-56 w-xl p-6 rounded-lg shadow-2xl flex flex-col space-x-2">
            <p class=" text-slate-900 font-Nunito text-md font-bold mb-5 mx-auto">CONSULTAR ESTADO DE RESULTADOS</p>
            <div class="flex flex-col">
                <div class="flex flex-col">
                    <form  @submit.prevent="estadoResultado">
